...
<c:if test="${searchType=='99'}">
	...
	<td rowspan="2">
		...
		<c:if test="${searchType=='99'}">
			<td>
				<input type="hidden" value="C" name="select"> 
				<h5>* 위의 선택하신 간행물의 종류가 등록하실 간행물이 맞는지 확인하여 주세요.</h5>
				<h5>* 등록하실 간행물의 정보를 입력하여 주세요</h5>
				출간 년도 [ ex) 16 년도 ] :<input type="text" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' style='ime-mode:disabled;' maxlength="2" name="gan_Y"> 년도<br>
				출간 월 [ ex) 09 월호 ]:<input type="text" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' style='ime-mode:disabled;' maxlength="2" name="gan_M"> 월호
			</td>
		</c:if>
		</tr>
</table>
